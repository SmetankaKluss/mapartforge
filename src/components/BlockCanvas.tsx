            ...
            for (let sy = 0; sy < rasterScale; sy++) {
              for (let sx = 0; sx < rasterScale; sx++) {
                ...
                if (entry) {
                  ...
                  const ssx = entry.blockId * CELL + Math.floor(sx * CELL  / rasterScale);
                  const ssy = spriteY0          + Math.floor(sy * spriteH / rasterScale);
        ...
        if (showGrid) {
          ...
          if (mapsWide > 1 || mapsTall > 1) {
            ...
            for (let mx = 1; mx < mapsWide; mx++) {
              const px = mx * 128 * rasterScale;
              ctx.beginPath(); ctx.moveTo(px, 0); ctx.lineTo(px, height * rasterScale); ctx.stroke();
            }
            for (let my = 1; my < mapsTall; my++) {
              const py = my * 128 * rasterScale;
              ctx.beginPath(); ctx.moveTo(0, py); ctx.lineTo(width * rasterScale, py); ctx.stroke();
            }
          }
        }
