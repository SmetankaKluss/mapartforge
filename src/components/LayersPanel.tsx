  ...
  return (
    <div className="layers-panel">
      <div className="layers-panel-header">
        ...
        <button className="layers-add-btn" onClick={onAdd} title="Добавить слой" aria-label="Добавить слой"><IconGlyph icon={mkIcons.plus} /></button>
        <button className="layers-add-btn" onClick={onMergeDown} title="Слить вниз" aria-label="Слить вниз" disabled={layers.length <= 1}><IconGlyph icon={mkIcons.arrowDown} /></button>
        <button className="layers-add-btn" onClick={onMergeVisible} title="Слить видимые" aria-label="Слить видимые" disabled={layers.filter(l => l.visible && l.imageData).length <= 1}><IconGlyph icon={mkIcons.layer} /></button>
        {selectedIds.length > 1 && (
          <button className="layers-add-btn" onClick={() => { onCreateGroup(selectedIds); setSelectedIds([]); }} title="Создать группу" aria-label="Создать группу"><IconGlyph icon={mkIcons.package} /></button>
        )}
      </div>
              ...
              if (grp) {
                rendered.push(
                  <div key={`grp-${gid}`} className="layer-group-header">
                    <button className="layer-group-collapse" onClick={() => onToggleGroupCollapse(gid)} aria-expanded={!grp.collapsed} aria-label="Свернуть или развернуть группу"><IconGlyph icon={grp.collapsed ? mkIcons.chevronRight : mkIcons.chevronDown} /></button>
                    <span className="layer-group-name">{grp.name}</span>
                    <button className="layer-del-btn" onClick={() => onDeleteGroup(gid)} title="Разгруппировать" aria-label="Разгруппировать"><IconGlyph icon={mkIcons.deselect} /></button>
                  </div>,
                );
              }
            } else if (gid === null) {
            ...
            >
              ...
              <div className="layer-row-main">
                ...
                <div className="layer-row-top">
                  {/* Type icon */}
                  <span className="layer-type-icon" title={layer.isText ? 'Текстовый слой' : 'Слой'}><IconGlyph icon={layer.isText ? mkIcons.text : mkIcons.layer} /></span>
                  ...
                  <div className="layer-icon-btns">
                    ...
                    >
                      <IconGlyph icon={layer.visible ? mkIcons.eye : mkIcons.eyeOff} />
                    </button>
                    <button
                    ...
                    >
                      <IconGlyph icon={layer.locked ? mkIcons.lock : mkIcons.unlock} />
                    </button>
                    <button
                ...
                <div className="layer-row-bottom">
                  ...
                  <div className="layer-move-btns">
                    <button
                      ...
                      title="Выше"
                    ><IconGlyph icon={mkIcons.arrowUp} /></button>
                    <button
                      className="layer-move-btn"
                      onClick={e => { e.stopPropagation(); onMoveDown(layer.id); }}
                      title="Ниже"
                    ><IconGlyph icon={mkIcons.arrowDown} /></button>
                  </div>
                </div>
              </div>
            </div>
