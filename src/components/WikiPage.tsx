...
const COMPANION_GITHUB_RELEASE_URL = 'https://github.com/SmetankaKluss/mapkluss-companion/releases/latest';
...
const ARTICLE_TOC: Record<WikiArticleId, TocItem[]> = {
  ...
  'editing-tools': [
    ...
    { id: 'advanced-tools', labelRu: 'Паттерн и градиент', labelEn: 'Pattern and gradient' },
  ],
  'layers-projects': [
  ...
  'cloud-companion': [
    ...
    { id: 'companion-tools', labelRu: 'Что умеет мод', labelEn: 'What the mod does' },
  ],
  lens: [
    ...
    { id: 'lens-safety', labelRu: 'Кто видит Lens', labelEn: 'Who can see Lens' },
  ],
  'two-layer': [
...
function ArticleBody({ articleId, t, onArticle }: { articleId: WikiArticleId; t: Translate; onArticle: (id: WikiArticleId) => void }) {
  switch (articleId) {
        ...
        <>
          ...
          <section id="what-it-does">
            ...
            <ul className="wiki-feature-list">
              ...
              <li><IconGlyph icon={mkIcons.cloud} /><span><strong>Cloud</strong>{t(' — приватные арты и доступ по ссылке, версии, избранное, коллекции и передача результата в мод.', ' — private and unlisted arts, versions, favorites, collections, and direct handoff to the mod.')}</span></li>
    ...
    case 'first-art':
      return (
        <>
          ...
          <section id="configure"><h2>{t('2. Выбрать размер и режим', '2. Choose size and mode')}</h2><p>{t('Для первой работы возьми 1×1 или 2×2 карты. Оставь 2D, если важна простая стройка, или выбери 3D для большей глубины цвета. По умолчанию выбран Blue Noise 1×; сохранённые настройки имеют приоритет.', 'For a first project, use a 1×1 or 2×2 grid. Keep 2D for a simpler build, or choose 3D for a wider shade range. Blue Noise 1× is the default; saved settings take priority.')}</p></section>
    ...
    case 'image-processing':
      return (
        <>
          ...
          <section id="dithering"><h2>{t('Дизеринг', 'Dithering')}</h2><p>{t('Дизеринг смешивает доступные цвета блоков, чтобы имитировать промежуточные оттенки. Blue Noise поддерживает масштаб 1× и 2×. Также доступны Floyd–Steinberg, Stucki, Atkinson и другие методы. «Без дизеринга» сохраняет чистые однотонные области. KlussDither оставлен только для совместимости со старыми настройками и проектами.', 'Dithering mixes available block colours to imitate missing shades. Blue Noise supports 1× and 2× scales. Floyd–Steinberg, Stucki, Atkinson and other methods are also available. None preserves solid colour regions. KlussDither is retained only for compatibility with existing settings and projects.')}</p></section>
        </>
      );
    ...
    case 'editing-tools':
      return (
        <>
          ...
          <section id="advanced-tools"><h2>{t('Паттерн и градиент', 'Pattern and gradient')}</h2><p>{t('Паттерн повторяет настраиваемую пиксельную сетку и поддерживает непрерывные штрихи. Градиент использует собственные цветовые точки и может добавлять упорядоченный дизеринг.', 'Pattern repeats a configurable pixel grid and supports continuous strokes. Gradient uses its own colour stops and optional ordered dithering.')}</p><a className="wiki-inline-action" href="/">{t('Открыть редактор', 'Open editor')} <IconGlyph icon={mkIcons.arrowRight} /></a></section>
        </>
      );
    ...
    case 'cloud-companion':
      return (
        <>
          ...
          <section id="cloud-save"><h2>{t('Сохранить в Cloud', 'Save to Cloud')}</h2><p>{t('Открой меню аккаунта в редакторе, войди по email или Telegram и выбери «Сохранить арт». Можно задать название и доступ: приватный или по ссылке. Новые сохранения создают версии, но не публикуют арт в общий каталог.', 'Open the account menu in the editor, sign in with email or Telegram, and choose Save art. Set the title and access: private or unlisted. Later saves create versions but never publish the art to a public catalogue.')}</p></section>
          <section id="install-companion"><h2>{t('Установить Companion', 'Install Companion')}</h2><p>{t('Выбери Fabric JAR строго под свою версию Minecraft: 26.2, 1.21.11, 1.21.8 или 1.21.4. Fabric API обязателен; для работы со схемами отдельно установи совместимые Litematica и MaLiLib.', 'Choose the Fabric JAR matching Minecraft exactly: 26.2, 1.21.11, 1.21.8, or 1.21.4. Fabric API is required; schematic workflows also need matching Litematica and MaLiLib versions.')}</p><div className="wiki-link-row"><a href="/cloud">{t('Скачать на странице Cloud', 'Download from Cloud')}</a><a href={COMPANION_GITHUB_RELEASE_URL} target="_blank" rel="noreferrer">GitHub Releases</a></div></section>
          ...
          <section id="companion-tools"><h2>{t('Что умеет мод', 'What the mod does')}</h2><p>{t('Companion показывает Cloud-библиотеку и превью, устанавливает цельные или разбитые по картам Litematic, ведёт Build Tracker, сканирует карты и стены из рамок, запускает Lens и помогает AutoFrame правильно расставлять распознанные части арта. Все эти функции работают на клиенте и не дают моду права автоматически менять мир сервера.', 'Companion shows the Cloud library and previews, installs whole or per-map Litematic files, provides Build Tracker, scans maps and framed walls, runs Lens, and uses AutoFrame to place recognized art tiles correctly. These are client-side tools and do not give the mod permission to modify the server world automatically.')}</p></section>
        </>
      );
    ...
    case 'two-layer':
      return (
        <>
          <p className="wiki-lead">{t('Two-layer сохраняет оттенки классического 3D-арта, но после ручного пошагового демонтажа оставляет компактную плоскую конструкцию.', 'Two-layer preserves classic 3D map shades but leaves a compact flat structure after a guided manual removal process.')}</p>
          ...
          <section id="two-layer-export"><h2>{t('Подготовить арт', 'Prepare an art')}</h2><ol><li>{t('Выбери Two-layer в группе способов строительства.', 'Select Two-layer in the build-method control.')}</li><li>{t('Используй совместимую Java-версию и проверь безопасную палитру.', 'Use a supported Java version and review the safe palette.')}</li><li>{t('Скачай Two-layer ZIP либо сохрани арт в Cloud.', 'Download the Two-layer ZIP or save the art to Cloud.')}</li><li>{t('Поддерживаются мозаики от 1×1 до 10×10 карт; пакет содержит отдельную проверяемую часть для каждой карты.', 'Mosaics from 1×1 through 10×10 maps are supported; the bundle contains a separately verified part for every map.')}</li></ol></section>
    ...
    case 'shortcuts':
      return (
        <>
          ...
          <section id="tool-keys"><h2>{t('Инструменты', 'Tools')}</h2><div className="wiki-shortcut-list"><div><span><Key>B</Key></span><strong>{t('Кисть', 'Brush')}</strong></div><div><span><Key>E</Key></span><strong>{t('Пипетка', 'Eyedropper')}</strong></div><div><span><Key>Shift</Key> + {t('ЛКМ', 'LMB')}</span><strong>{t('Брать блоки пипеткой по ходу курсора', 'Sample blocks along the cursor with the eyedropper')}</strong></div><div><span><Key>F</Key></span><strong>{t('Заливка', 'Fill')}</strong></div><div><span><Key>X</Key></span><strong>{t('Ластик', 'Eraser')}</strong></div><div><span><Key>P</Key> / <Key>G</Key></span><strong>{t('Паттерн / градиент', 'Pattern / gradient')}</strong></div><div><span><Key>Ctrl/⌘ Z</Key></span><strong>{t('Отменить', 'Undo')}</strong></div></div></section>
