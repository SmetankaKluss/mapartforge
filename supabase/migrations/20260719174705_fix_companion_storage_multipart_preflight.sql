-- Multipart uploads expose the size of the complete HTTP request during the
-- Storage RLS preflight, not the exact file size. Keep the reservation as the
-- authoritative source for quota accounting and accept only a small bounded
-- multipart envelope. Completed-object metadata must still match exactly.
