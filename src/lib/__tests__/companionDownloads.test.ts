...
describe('companion downloads', () => {
  it('ships matching checksums and production-only 0.14.1 jars for every link', async () => {
    const base = new URL('../../../public/downloads/mod/', import.meta.url);
    const checksums = new Map(readFileSync(new URL('SHA256SUMS-0.14.1.txt', base), 'utf8')
      .trim().split(/\r?\n/).map(line => {
    ...
    for (const option of COMPANION_MOD_VERSION_OPTIONS) {
      ...
      expect(metadata.version).toBe('0.14.1');
  ...
  it('keeps the four supported Fabric targets in one canonical list', () => {
    ...
    expect(COMPANION_MOD_VERSION_OPTIONS.every(option => option.href.endsWith('20260913-companion-0-14-1'))).toBe(true);
  });
  ...
  it('uses the exact target when supported and a safe 1.21.4 fallback otherwise', () => {
    ...
    expect(companionDownloadOption('1.21.11').filename).toContain('1.21.11-0.14.1.jar');
  });
});
