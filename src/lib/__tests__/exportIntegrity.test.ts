...
describe('export integrity', () => {
  ...
  it('counts only opaque palette-matched pixels in material exports', async () => {
    ...
    const csv = formatMaterialsAsCSV(materials, { wide: 1, tall: 1 });
    expect(csv).toContain('Block Name,NBT Name,Count,Stacks (64),Shulker Boxes (1728)');
    expect(csv).toContain('"Grass Block","grass_block",2');
    expect(csv).toContain('Total blocks,3');
  });
});
