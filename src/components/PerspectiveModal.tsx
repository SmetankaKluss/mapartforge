...
function makeTextureMaterial(
  ...
  const cachedTexture = textureCache.get(textureName);
  if (!cachedTexture) {
    new THREE.TextureLoader()
      .setCrossOrigin('anonymous')
      .load(
...
export function PerspectiveModal({
  ...
  useEffect(() => {
    ...
    Object.assign(window as object, { __camera: camera, __renderer: renderer, __render: render });
    ...
    // Flat, bright lighting to match Minecraft's in-game look: strong neutral
    // ambient + a soft top key so block tops read slightly brighter than sides,
    // and a low neutral fill so back/bottom faces are never pitch black. No cold
    // tint — Minecraft interior light is warm-neutral, not blue.
