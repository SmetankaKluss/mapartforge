...
function setupExactColourCache() {
  ...
  for (const [colourSetId, colourSet] of Object.entries(coloursJSON)) {
    if (selectedBlocks[colourSetId] === "-1") continue;
    for (const [toneKey, toneRGB] of Object.entries(colourSet.tonesRGB)) {
      const RGBBinary = (toneRGB[0] << 16) + (toneRGB[1] << 8) + toneRGB[2];
      if (!exactColourCache.has(RGBBinary)) exactColourCache.set(RGBBinary, { colourSetId, tone: toneKey });
    }
  }
}
...
function getMapartImageDataAndMaterials() {
  ...
  for (let i = 0; i < canvasImageData.data.length; i += 4) {
    ...
    if (
      ...
      if (prequantized) {
        closestColourSetIdAndTone = exactRGBToColourSetIdAndTone(oldPixel);
        if (!closestColourSetIdAndTone) throw new Error(`Processed color is outside the selected palette: ${oldPixel.join(",")}`);
      } else switch (chosenDitherMethod.uniqueId) {
...
onmessage = (e) => {
  ...
  prequantized = e.data.body.prequantized === true;
